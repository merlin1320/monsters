...
export function Monsters() {
  ...
  const [selectedMonster, setSelectedMonster] = useState<Monster>({} as Monster);
  ...
  const handleSave = async () => {
    ...
    try {
      await axios.patch(`${api_url}/monster/${selectedMonster.id}/attributes`, selectedMonster.attributes);
      setModalOpen(false);
      setSelectedMonster({} as Monster);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginTop: 40 }}>
      ...
      {/* Removed old local download logic. Button now only calls the API endpoint for download. */}
      <button
        style={{ marginTop: 24 }}
        onClick={async () => {
          try {
            ...
            alert('Failed to export monster list.');
          }
        }}
      >
        Download Monster List (JSON)
      </button>
      {/* Modal */}
      {modalOpen && selectedMonster && (
        ...
        >
          ...
          >
            <h2 style={{ color: '#fff', marginBottom: 20 }}>Edit Monster</h2>
